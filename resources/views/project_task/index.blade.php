                                        ...
                                        <div class="d-flex align-items-center justify-content-between p-2">
                                            <div class="badge-xs badge bg-{{\App\Models\ProjectTask::$priority_color[$taskDetail->priority]}} p-2 px-3 rounded">{{ __(\App\Models\ProjectTask::$priority[$taskDetail->priority]) }}</div>
                                             <div class="user-group">
                                                    @if(!empty($taskDetail->end_date) && $taskDetail->end_date != '0000-00-00' && $taskDetail->end_date !='1970-01-01')<span data-bs-toggle="tooltip" title="{{__('End Date')}}" @if(strtotime($taskDetail->end_date) < time())class="text-danger"@endif>{{ Utility::getDateFormated($taskDetail->end_date) }}</span>@else <span data-bs-toggle="tooltip" title="{{__('End Date')}}"> - </span>@endif
                                                </div>
                                        </div>
